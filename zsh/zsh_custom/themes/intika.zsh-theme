...
PROMPT='$(ssh_connection)%{$fg[white]%}[%T][%m:%n%{$reset_color%}:%{$fg[green]%}%c%{$reset_color%}%{$fg[white]%}$(git_prompt_info)]$(create_space_filled_string)%{$fg[white]%}[%~]%{$reset_color%}%(?,$FG[red][$?],$FG[green][$?])%{$reset_color%}
